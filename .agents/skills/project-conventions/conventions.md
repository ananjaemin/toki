...
Use it before editing `menubar/Toki/**/*.swift`, `menubar/TokiTests/**/*.swift`,
resources, or project configuration.
...
- Keep responsibilities aligned with the existing tree:
  - `menubar/Toki/App`: app entry, app delegate, menu bar lifecycle.
  - `menubar/Toki/Domain`: pure usage and security audit domain models/builders.
  - `menubar/Toki/Infrastructure`: readers, SQLite access, filesystem access, caches,
    activity monitoring, pricing data, and scanner implementations.
  - `menubar/Toki/Features`: SwiftUI screens, view models, settings, refresh
    coordination, exports, and feature-specific presentation logic.
  - `menubar/TokiTests`: unit tests and local test support.
...
- Prefer editing `menubar/project.yml` for project structure/config changes,
  then run `xcodegen generate` from `menubar/`.
